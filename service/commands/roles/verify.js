...
const { verifyProfile, findProfile } = require('../../../api/clash/verification')
const { tagVerified, alreadyTaken, insertVerification } = require('../../../api/mongo/verify/connections')
const { getInvalidApiTokenEmbed, getInvalidTagEmbed, getValidVerificationEmbed } = require('../../../utils/embeds/verify')
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!findProfileResponse.response.found) {
            await interaction.reply({embeds: [getInvalidTagEmbed()], ephemeral: true})
            return
        }
        ...
        if (!isValid) {
            await interaction.reply({embeds: [getInvalidApiTokenEmbed()], ephemeral: true})
            return
        }
        ...
        if (await tagVerified(tag)) {
            if(await alreadyTaken(tag, interaction.member.id)) {
                ...
                await interaction.reply({embeds: [setRoles(profileData, interaction.member)], ephemeral: true})
                return
            }
        } else {
...
const setRoles = (playerData, user) => {
    ...
    const achieved = {
        ...
        farmersRUs: playerAchievement[5].value >= 2000000000 
    ...
    const noRoles = thLevel === 0 && Object.values(achieved).every(val => val === false)

    return getValidVerificationEmbed(createValidVerificationEmbedDescription(achieved, thLevel, noRoles))
}

const createValidVerificationEmbedDescription = (achieved, thLevel, noRoles) => {
    if (noRoles) return `• Not eligible for any roles\n`

    let thEmbedDesc = ""
    for (thRole in roles.townhall) {
        if (thRole.lvl === thLevel) thEmbedDesc = `${thRole.icon} <@&${thRole.roleid}> added!\n`
        break
    }

    return (
        achieved.legends ? `${roles.prestige.legends.icon} <@&${roles.prestige.legends.roleid}> added!\n` : `` +
        achieved.starLord ? `${roles.prestige.starlord.icon} <@&${roles.prestige.starlord.roleid}> added!\n` : `` +
        achieved.farmersRUs ? `${roles.prestige.farmersrus.icon} <@&${roles.prestige.farmersrus.roleid}> added!\n` : `` +
        achieved.blackGold ? `${roles.prestige.blackgold.icon} <@&${roles.prestige.blackgold.roleid}> added!\n` : `` +
        achieved.masterBuilder ? `${roles.prestige.masterbuilder.icon} <@&${roles.prestige.masterbuilder.roleid}> added!\n` : `` +
        achieved.philanthropist ? `${roles.prestige.philanthropist.icon} <@&${roles.prestige.philanthropist.roleid}> added!\n` : `` +
        achieved.alchemist ? `${roles.prestige.alchemist.icon} <@&${roles.prestige.alchemist.roleid}> added!\n` : `` +
        achieved.rockSolid ? `${roles.prestige.rocksolid.icon} <@&${roles.prestige.rocksolid.roleid}> added!\n` : `` +
        achieved.greenThumb ? `${roles.prestige.greenthumb.icon} <@&${roles.prestige.greenthumb.roleid}> added!\n` : `` +
        achieved.leagueLord ? `${roles.prestige.leaguelord.icon} <@&${roles.prestige.leaguelord.roleid}> added!\n` : `` +
        achieved.masterGamer ? `${roles.prestige.mastergamer.icon} <@&${roles.prestige.mastergamer.roleid}> added!\n` : `` +
        roles.townhall > 0 ? thEmbedDesc :``
    )
}
...
const addAchievementRoles = (user, achieved) => {
    ...
    if (achieved.masterBuilder) user.roles.add(roles.prestige.masterbuilder.roleid)
    if (achieved.philanthropist) user.roles.add(roles.prestige.philanthropist.roleid)
...
const addTownhall = (player, user) => {
    ...
    if (newThLevel > oldThLevel) {
        ...
        for (thRole in roles.townhall) {
            if (thRole.lvl === newThLevel){
                user.roles.add(thRole.roleid);
                return newThLevel
            }
        }
    }
...
const removeTH = (user, lvl) => {   
    const user_roles = user.roles.cache; 
    for (i in roles.townhall){
        if(user_roles.has(roles.townhall[i].roleid) && roles.townhall[i].lvl < lvl){
