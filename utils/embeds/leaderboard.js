...
const getLegendaryLeaderboard = (topLegends, legendParticipantCount, index, pageSize) => {
    ...
    for (let i = topRankRange; i <= bottomRankRange; i++){
        embed.addFields({
            name: `Rank ${(index*pageSize) + i+1} - ${topLegends[i].trophiesLegends} 🏆`,
...
const getBuilderLeaderboard = (topBuilders, builderParticipantCount, index, pageSize) => {
    const topRankRange = (index * pageSize)
    const bottomRankRange = Math.min((index * pageSize) + pageSize, builderParticipantCount)
    const embed = new EmbedBuilder()
    .setTitle('Builder base leaderboard')
    .setColor('#0B0B3B');
    for (let i = topRankRange; i < bottomRankRange; i++){
        embed.addFields({
            name: `Rank ${i+1} - ${topBuilders[i].trophiesBuilders} 🏆`,
